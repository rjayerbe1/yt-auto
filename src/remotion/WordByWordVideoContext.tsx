...
import * as Styles from './styles/VideoStylesContext';

// Define which style to use (change this number from 1-30 to test different styles)
const SELECTED_STYLE = 15; // Change this to test different styles!
...
export const WordByWordVideo: React.FC<WordByWordVideoProps> = ({
  ...
  // Get current segment and word with context
  const getCurrentContext = () => {
    for (const segment of segments) {
      if (segment.wordTimings) {
        // Find if we're in this segment's time range
        ...
        if (currentWordIndex !== -1) {
          // Get context: previous, current, and next words
          const contextRange = 5; // Show 5 words before and after
          const startIdx = Math.max(0, currentWordIndex - contextRange);
          const endIdx = Math.min(segmentWords.length, currentWordIndex + contextRange + 1);
          
          const contextWords = segmentWords.slice(startIdx, endIdx).map((w, idx) => ({
            word: w.word,
            isActive: startIdx + idx === currentWordIndex,
            opacity: 1 - Math.abs(startIdx + idx - currentWordIndex) * 0.15, // Fade out words further from current
          }));
          
          return {
            words: contextWords,
            currentWord: segmentWords[currentWordIndex],
            fullPhrase: segment.text,
  ...
  const context = useMemo(() => getCurrentContext(), [currentTime, segments]);

  // Calculate animation for current word
  const wordAnimation = useMemo(() => {
    if (!context || !context.currentWord) return { scale: 0, opacity: 0 };
    
    const wordStartFrame = Math.floor(context.currentWord.startTime * fps);
    const wordEndFrame = Math.floor(context.currentWord.endTime * fps);
    const frameSinceStart = frame - wordStartFrame;
    const wordDurationFrames = wordEndFrame - wordStartFrame;
    
    // Smooth fade in
    if (frameSinceStart < 3) {
      const progress = frameSinceStart / 3;
      return {
  ...
  return (
    ...
    >
      ...
      {/* Render selected style with context */}
      <SelectedStyle 
        context={context}
